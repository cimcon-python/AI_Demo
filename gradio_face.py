...
def validate_image(image):
    ...
    # 1️⃣ Check brightness (Lighting Condition)
    ...
    # 2️⃣ Check if a face is detected
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    results = face_detection.process(img_rgb)
    
    if not results.detections:
        return False, "No face detected. Please align your face inside area."

    # 3️⃣ Check if face is inside a round boundary    
    ...
    for detection in results.detections:
        ...
        if distance > radius:
            return False, "Face is not properly aligned. Please adjust your position."
